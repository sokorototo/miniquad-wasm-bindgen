...
impl From<BlendFactor> for MTLBlendFactor {
	fn from(factor: BlendFactor) -> Self {
		match factor {
			...
			BlendFactor::OneMinusValue(BlendValue::SourceColor) => MTLBlendFactor::OneMinusSourceColor,
			BlendFactor::OneMinusValue(BlendValue::SourceAlpha) => MTLBlendFactor::OneMinusSourceAlpha,
			BlendFactor::OneMinusValue(BlendValue::DestinationColor) => MTLBlendFactor::OneMinusDestinationColor,
			BlendFactor::OneMinusValue(BlendValue::DestinationAlpha) => MTLBlendFactor::OneMinusDestinationAlpha,
	...
	pub fn new() -> MetalContext {
		unsafe {
			...
			if false {
				...
				let capture_descriptor = msg_send_![msg_send_![class![MTLCaptureDescriptor], alloc], init];
			...
			#[cfg(target_os = "macos")]
			let options = { MTLResourceOptions::CPUCacheModeWriteCombined | MTLResourceOptions::StorageModeManaged };
			#[cfg(target_os = "ios")]
...
impl RenderingBackend for MetalContext {
	...
	fn texture_resize(&mut self, _texture: TextureId, _width: u32, _height: u32, _bytes: Option<&[u8]>) {
		unimplemented!()
	}
	...
	fn clear(&mut self, color: Option<(f32, f32, f32, f32)>, depth: Option<f32>, stencil: Option<i32>) {
		...
		self.begin_default_pass(PassAction::Clear { color, depth, stencil });
		self.end_render_pass();
	}

	fn new_render_pass_mrt(&mut self, color_img: &[TextureId], depth_img: Option<TextureId>) -> RenderPass {
		unsafe {
			let render_pass_desc = msg_send_![class!(MTLRenderPassDescriptor), renderPassDescriptor];
	...
	fn new_shader(&mut self, shader: ShaderSource, _meta: ShaderMeta) -> Result<ShaderId, ShaderError> {
		unsafe {
			...
			let shader = ShaderInternal { vertex_function, fragment_function };
			self.shaders.push(shader);
			Ok(ShaderId(self.shaders.len() - 1))
		}
	}

	fn new_texture(&mut self, access: TextureAccess, bytes: TextureSource, params: TextureParams) -> TextureId {
		...
		match bytes {
			...
			TextureSource::Bytes(bytes) => {
				assert_eq!(params.format.size(params.width, params.height) as usize, bytes.len());
				...
				for (n, face) in array.iter().enumerate() {
					for (mipmap_level, bytes) in face.iter().enumerate() {
						...
						let region = MTLRegion {
							origin: MTLOrigin { x: 0 as u64, y: 0 as u64, z: 0 },
							size: MTLSize {
								width: params.width as u64,
								height: params.height as u64,
								depth: 1,
	...
	fn new_pipeline(&mut self, buffer_layout: &[BufferLayout], attributes: &[VertexAttribute], shader: ShaderId, params: PipelineParams) -> Pipeline {
		unsafe {
			...
			let vertex_descriptor: ObjcId = msg_send![class!(MTLVertexDescriptor), vertexDescriptor];
			...
			for (i, a) in attributes.iter().enumerate() {
				...
				attribute(i as u64, a.buffer_index as u64 + 1, *offset, a.format.into());
				*offset += a.format.size_bytes() as u64;
			}
			for (i, buffer) in buffer_layout.iter().enumerate() {
				layout(i as u64 + 1, buffer.step_func, if buffer.stride == 0 { offsets[i] } else { buffer.stride as u64 }, buffer.step_rate as u64);
			}
	...
	fn apply_pipeline(&mut self, pipeline: &Pipeline) {
		assert!(self.render_encoder.is_some(), "apply_pipeline before begin_pass");
	...
	fn apply_bindings_from_slice(&mut self, vertex_buffers: &[BufferId], index_buffer: BufferId, textures: &[TextureId]) {
		assert!(self.render_encoder.is_some(), "apply_bindings before begin_pass");
		...
		unsafe {
			...
			if img_count > 0 {
				for (n, img) in textures.iter().enumerate() {
					let Texture { sampler, texture, .. } = self.textures.get(*img);
	...
	fn apply_uniforms_from_bytes(&mut self, uniform_ptr: *const u8, size: usize) {
		assert!(self.current_pipeline.is_some(), "apply_uniforms before apply_pipeline");
		assert!(self.render_encoder.is_some(), "apply_uniforms before begin_pass");
		...
		unsafe {
			...
			std::ptr::copy(uniform_ptr as _, dest.add(self.current_ub_offset as usize), size);
		...
		unsafe {
			...
			let (descriptor, _, _) = match pass {
				...
				Some(pass) => {
					...
					(pass.render_pass_desc, self.textures.get(texture).params.width as f64, self.textures.get(texture).params.height as f64)
				}
			};
	...
	fn end_render_pass(&mut self) {
		assert!(self.render_encoder.is_some(), "end_render_pass unpaired with begin_render_pass!");
