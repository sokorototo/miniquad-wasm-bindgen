...
pub fn define_app_delegate() -> *const Class {
	...
	unsafe {
		decl.add_method(sel!(applicationShouldTerminateAfterLastWindowClosed:), yes1 as extern "C" fn(&Object, Sel, ObjcId) -> BOOL);
	}
...
pub fn define_cocoa_window_delegate() -> *const Class {
	...
	unsafe {
		decl.add_method(sel!(windowShouldClose:), window_should_close as extern "C" fn(&Object, Sel, ObjcId) -> BOOL);

		decl.add_method(sel!(windowDidResize:), window_did_resize as extern "C" fn(&Object, Sel, ObjcId));
		decl.add_method(sel!(windowDidChangeScreen:), window_did_change_screen as extern "C" fn(&Object, Sel, ObjcId));
		decl.add_method(sel!(windowDidEnterFullScreen:), window_did_enter_fullscreen as extern "C" fn(&Object, Sel, ObjcId));
		decl.add_method(sel!(windowDidExitFullScreen:), window_did_exit_fullscreen as extern "C" fn(&Object, Sel, ObjcId));
	}
...
unsafe fn get_proc_address(name: *const u8) -> Option<unsafe extern "C" fn()> {
	...
	if OPENGL.is_null() {
		OPENGL = libc::dlopen(b"/System/Library/Frameworks/OpenGL.framework/Versions/Current/OpenGL\0".as_ptr() as _, libc::RTLD_LAZY);
	}
...
unsafe fn view_base_decl(decl: &mut ClassDecl) {
	...
	extern "C" fn reset_cursor_rects(this: &Object, _sel: Sel) {
		...
		unsafe {
			let cursor_id = {
				...
				let cursor_id = *payload.cursors.entry(current_cursor).or_insert_with(|| load_mouse_cursor(current_cursor.clone()));
	...
	extern "C" fn flags_changed(this: &Object, _sel: Sel, event: ObjcId) {
		fn produce_event(payload: &mut MacosDisplay, keycode: crate::KeyCode, mods: crate::KeyMods, old_pressed: bool, new_pressed: bool) {
	...
	decl.add_method(sel!(canBecomeKey), yes as extern "C" fn(&Object, Sel) -> BOOL);
	decl.add_method(sel!(acceptsFirstResponder), yes as extern "C" fn(&Object, Sel) -> BOOL);
	...
	decl.add_method(sel!(mouseMoved:), mouse_moved as extern "C" fn(&Object, Sel, ObjcId));
	decl.add_method(sel!(mouseDragged:), mouse_moved as extern "C" fn(&Object, Sel, ObjcId));
	decl.add_method(sel!(rightMouseDragged:), mouse_moved as extern "C" fn(&Object, Sel, ObjcId));
	decl.add_method(sel!(otherMouseDragged:), mouse_moved as extern "C" fn(&Object, Sel, ObjcId));
	decl.add_method(sel!(mouseDown:), mouse_down as extern "C" fn(&Object, Sel, ObjcId));
	decl.add_method(sel!(mouseUp:), mouse_up as extern "C" fn(&Object, Sel, ObjcId));
	decl.add_method(sel!(rightMouseDown:), right_mouse_down as extern "C" fn(&Object, Sel, ObjcId));
	decl.add_method(sel!(rightMouseUp:), right_mouse_up as extern "C" fn(&Object, Sel, ObjcId));
	decl.add_method(sel!(otherMouseDown:), other_mouse_down as extern "C" fn(&Object, Sel, ObjcId));
	decl.add_method(sel!(otherMouseUp:), other_mouse_up as extern "C" fn(&Object, Sel, ObjcId));
	decl.add_method(sel!(scrollWheel:), scroll_wheel as extern "C" fn(&Object, Sel, ObjcId));
	decl.add_method(sel!(keyDown:), key_down as extern "C" fn(&Object, Sel, ObjcId));
	decl.add_method(sel!(flagsChanged:), flags_changed as extern "C" fn(&Object, Sel, ObjcId));
...
pub fn define_opengl_view_class() -> *const Class {
	...
	unsafe {
		...
		decl.add_method(sel!(timerFired:), timer_fired as extern "C" fn(&Object, Sel, ObjcId));

		decl.add_method(sel!(prepareOpenGL), prepare_open_gl as extern "C" fn(&Object, Sel));
...
unsafe fn create_opengl_view(window_frame: NSRect, sample_count: i32, high_dpi: bool) -> ObjcId {
	...
	let glpixelformat_obj: ObjcId = msg_send![glpixelformat_obj, initWithAttributes: attrs.as_ptr()];
	...
	F: 'static + FnOnce() -> Box<dyn EventHandler>,
{
	...
	let window_masks =
		NSWindowStyleMask::NSTitledWindowMask as u64 | NSWindowStyleMask::NSClosableWindowMask as u64 | NSWindowStyleMask::NSMiniaturizableWindowMask as u64 | NSWindowStyleMask::NSResizableWindowMask as u64;
