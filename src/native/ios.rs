...
pub fn define_glk_or_mtk_view(superclass: &Class) -> *const Class {
	...
	unsafe {
		...
		decl.add_method(sel!(touchesBegan: withEvent:), touches_began as extern "C" fn(&Object, Sel, ObjcId, ObjcId));
		decl.add_method(sel!(touchesMoved: withEvent:), touches_moved as extern "C" fn(&Object, Sel, ObjcId, ObjcId));
		decl.add_method(sel!(touchesEnded: withEvent:), touches_ended as extern "C" fn(&Object, Sel, ObjcId, ObjcId));
		decl.add_method(sel!(touchesCanceled: withEvent:), touches_canceled as extern "C" fn(&Object, Sel, ObjcId, ObjcId));
		decl.add_method(sel!(processMessage:), process_message as extern "C" fn(&Object, Sel, ObjcId));
	}
...
unsafe fn get_proc_address(name: *const u8) -> Option<unsafe extern "C" fn()> {
	...
	if OPENGL.is_null() {
		OPENGL = libc::dlopen(b"/System/Library/Frameworks/OpenGLES.framework/OpenGLES\0".as_ptr() as _, libc::RTLD_LAZY);
	}
...
pub fn define_glk_or_mtk_view_dlg(superclass: &Class) -> *const Class {
	...
	extern "C" fn draw_in_rect(this: &Object, _: Sel, _: ObjcId, _: ObjcId) {
		...
		let (screen_width, screen_height) = if high_dpi {
			(screen_rect.size.width as i32 * 2, screen_rect.size.height as i32 * 2)
		} else {
			let content_scale_factor: f64 = unsafe { msg_send![payload.view, contentScaleFactor] };
			((screen_rect.size.width * content_scale_factor) as i32, (screen_rect.size.height * content_scale_factor) as i32)
		};

		if native_display().lock().unwrap().screen_width != screen_width || native_display().lock().unwrap().screen_height != screen_height {
			{
	...
	unsafe {
		decl.add_method(sel!(glkView: drawInRect:), draw_in_rect as extern "C" fn(&Object, Sel, ObjcId, ObjcId));

		decl.add_method(sel!(drawInMTKView:), draw_in_rect2 as extern "C" fn(&Object, Sel, ObjcId));
	}
...
pub fn define_app_delegate() -> *const Class {
	...
	extern "C" fn did_finish_launching_with_options(_: &Object, _: Sel, _: ObjcId, _: ObjcId) -> BOOL {
		unsafe {
...
fn define_textfield_dlg() -> *const Class {
	...
	extern "C" fn should_change_characters_in_range(this: &Object, _: Sel, _textfield: ObjcId, _range: NSRange, string: ObjcId) -> BOOL {
		let payload = get_window_payload(this);
            ...
            if len > 0 {
                for i in 0..len {
					...
					match c {
						...
						10 => {
							if let Some(ref mut event_handler) = payload.event_handler {
								event_handler.key_down_event(crate::event::KeyCode::Enter, Default::default(), false);
								event_handler.key_up_event(crate::event::KeyCode::Enter, Default::default());
							}
						}
						32 => {
							if let Some(ref mut event_handler) = payload.event_handler {
								event_handler.char_event(' ', Default::default(), false);
								event_handler.key_down_event(crate::event::KeyCode::Space, Default::default(), false);
								event_handler.key_up_event(crate::event::KeyCode::Space, Default::default());
							}
						}
						_ => {}
					}
				...
				if let Some(ref mut event_handler) = payload.event_handler {
					event_handler.key_down_event(crate::event::KeyCode::Backspace, Default::default(), false);
					event_handler.key_up_event(crate::event::KeyCode::Backspace, Default::default());
				}
			}
		}
		NO
	...
	unsafe {
		decl.add_method(sel!(keyboardWasShown:), keyboard_was_shown as extern "C" fn(&Object, Sel, ObjcId));
		decl.add_method(sel!(keyboardWillBeHidden:), keyboard_will_be_hidden as extern "C" fn(&Object, Sel, ObjcId));
		...
		decl.add_method(
			...
			should_change_characters_in_range as extern "C" fn(&Object, Sel, ObjcId, NSRange, ObjcId) -> BOOL,
		);
	}
	decl.add_ivar::<*mut c_void>("display_ptr");
	return decl.register();
...
pub fn load_file<F: FnOnce(crate::fs::Response) + 'static>(path: &str, on_loaded: F) {
	...
	unsafe {
		let nsstring = apple_util::str_to_nsstring(&format!("loading: {} {}", path_without_extension, extension));
