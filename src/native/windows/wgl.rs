...
type GetPixelFormatAttribivARB = extern "system" fn(_: HDC, _: INT, _: INT, _: UINT, _: *const INT, _: *mut INT) -> bool;
...
pub unsafe fn gl_choose_fbconfig(desired: &mut GlFbconfig, alternatives: &[GlFbconfig]) -> Option<usize> {
	let mut missing: i32;
...
impl Wgl {
	pub(crate) unsafe fn new(display: &mut WindowsDisplay) -> Wgl {
		...
		pfd.cColorBits = 24;
		if SetPixelFormat(display.msg_dc, ChoosePixelFormat(display.msg_dc, &pfd), &pfd) == 0 {
		...
		let GetExtensionsStringEXT: Option<GetExtensionsStringEXT> = get_wgl_proc_address(&mut display.libopengl32, "wglGetExtensionsStringEXT");
		let GetExtensionsStringARB: Option<GetExtensionsStringARB> = get_wgl_proc_address(&mut display.libopengl32, "wglGetExtensionsStringARB");
		let CreateContextAttribsARB: Option<CreateContextAttribsARB> = get_wgl_proc_address(&mut display.libopengl32, "wglCreateContextAttribsARB");
		let SwapIntervalEXT: Option<SwapIntervalEXT> = get_wgl_proc_address(&mut display.libopengl32, "wglSwapIntervalEXT");
		let GetPixelFormatAttribivARB: Option<GetPixelFormatAttribivARB> = get_wgl_proc_address(&mut display.libopengl32, "wglGetPixelFormatAttribivARB");
	...
	unsafe fn wgl_attrib(&self, display: &mut WindowsDisplay, pixel_format: i32, attrib: i32) -> i32 {
		let mut value = 0;
	...
	unsafe fn wgl_find_pixel_format(&self, display: &mut WindowsDisplay, sample_count: i32) -> u32 {
		...
		for i in 0..native_count {
			...
			*u = Default::default();
			if self.wgl_attrib(display, n, WGL_SUPPORT_OPENGL_ARB as _) == 0 || self.wgl_attrib(display, n, WGL_DRAW_TO_WINDOW_ARB as _) == 0 {
				continue;
			}
			if self.wgl_attrib(display, n, WGL_PIXEL_TYPE_ARB as _) != WGL_TYPE_RGBA_ARB as _ {
				continue;
			}
			if self.wgl_attrib(display, n, WGL_ACCELERATION_ARB as _) == WGL_NO_ACCELERATION_ARB as _ {
				continue;
			}
	...
	pub(crate) unsafe fn create_context(&mut self, display: &mut WindowsDisplay, sample_count: i32, swap_interval: i32) -> HGLRC {
		let pixel_format = self.wgl_find_pixel_format(display, sample_count);
		if 0 == pixel_format {
			panic!("WGL: Didn't find matching pixel format.");
		}
		let mut pfd: PIXELFORMATDESCRIPTOR = std::mem::zeroed();
		if DescribePixelFormat(display.dc, pixel_format as _, std::mem::size_of_val(&pfd) as _, &mut pfd as *mut _ as _) == 0 {
		...
		if gl_ctx.is_null() {
			...
			gl_ctx = self.CreateContextAttribsARB.unwrap()(display.dc, std::ptr::null_mut(), attrs.as_ptr() as *const _);
		}
		if gl_ctx.is_null() {
