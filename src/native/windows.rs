...
impl WindowsDisplay {
	...
	fn set_window_size(&mut self, new_width: u32, new_height: u32) {
		...
		if unsafe { AdjustWindowRectEx(&mut rect as *mut _ as _, win_style, false as _, win_style_ex) } != 0 {
			final_new_width = (rect.right - rect.left) as _;
			final_new_height = (rect.bottom - rect.top) as _;
		}

		unsafe { SetWindowPos(self.wnd, HWND_TOP, x, y, final_new_width as i32, final_new_height as i32, SWP_NOMOVE) };
	}
	...
	fn set_fullscreen(&mut self, fullscreen: bool) {
		...
		unsafe {
			...
			if self.fullscreen {
				SetWindowPos(self.wnd, HWND_TOP, 0, 0, GetSystemMetrics(SM_CXSCREEN), GetSystemMetrics(SM_CYSCREEN), SWP_FRAMECHANGED);
			} else {
				let (w, h) = {
...
fn get_win_style(is_fullscreen: bool, is_resizable: bool) -> DWORD {
	if is_fullscreen {
		...
		let mut win_style: DWORD = WS_CLIPSIBLINGS | WS_CLIPCHILDREN | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX;
...
unsafe fn update_clip_rect(hwnd: HWND) {
	...
	SetRect(&mut rect as *mut _ as _, upper_left.x, upper_left.y, lower_right.x, lower_right.y);
...
unsafe fn create_win_icon_from_image(width: u32, height: u32, colors: &[u8]) -> Option<HICON> {
	...
	let color = CreateDIBSection(dc, &bi as *const _ as *const BITMAPINFO, DIB_RGB_COLORS, &mut target, std::ptr::null_mut(), 0);
...
unsafe fn set_icon(wnd: HWND, icon: &Icon) {
	...
	let big_icon = if big_icon_w * big_icon_h >= 64 * 64 { (&icon.big[..], 64, 64) } else { (&icon.medium[..], 32, 32) };
...
unsafe fn create_window(window_title: &str, fullscreen: bool, resizable: bool, width: i32, height: i32) -> (HWND, HDC) {
...
impl WindowsDisplay {
	...
	unsafe fn init_dpi(&mut self, high_dpi: bool) {
		...
		if self.dpi_aware {
			...
			let mut dpix: UINT = 0;
			let mut dpiy: UINT = 0;
			let hr = GetDpiForMonitor(hm, MDT_EFFECTIVE_DPI, &mut dpix as *mut _ as _, &mut dpiy as *mut _ as _);
			assert_eq!(hr, 0);
	...
	F: 'static + FnOnce() -> Box<dyn EventHandler>,
{
	unsafe {
		...
		let (wnd, dc) = create_window(&conf.window_title, conf.fullscreen, conf.window_resizable, conf.window_width as _, conf.window_height as _);
		...
		let gl_ctx = wgl.create_context(&mut display, conf.sample_count, conf.platform.swap_interval.unwrap_or(1));
