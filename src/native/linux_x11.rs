...
impl X11Display {
	unsafe fn process_event(&mut self, event: &mut XEvent, event_handler: &mut dyn EventHandler) {
		match (*event).type_0 {
			2 => {
				...
				(self.libx11.XLookupString)(&mut (*event).xkey, std::ptr::null_mut(), 0 as libc::c_int, &mut keysym, std::ptr::null_mut());
	...
	unsafe fn set_fullscreen(&mut self, window: Window, fullscreen: bool) {
		...
		let wm_state = (self.libx11.XInternAtom)(self.display, b"_NET_WM_STATE\x00" as *const u8 as *const _, false as _);
		let wm_fullscreen = (self.libx11.XInternAtom)(self.display, b"_NET_WM_STATE_FULLSCREEN\x00" as *const u8 as *const _, false as _);
		...
		// someone on stackoverflow mentioned that this is not working on ubuntu/unity though
		{
			...
			(self.libx11.XChangeProperty)(self.display, window, wm_state, 4 as _, 32, PropModeReplace, atoms.as_mut_ptr() as *mut _ as *mut _, 1);
		...
		// send ClientMessage to the window with request to change property to fullscreen
		{
			...
			(self.libx11.XSendEvent)(self.display as _, self.root, false as _, (1048576 | 131072) as _, &mut ev as *mut XClientMessageEvent as *mut _);
		}
	}
...
unsafe fn glx_main_loop<F>(mut display: X11Display, conf: &crate::conf::Conf, f: &mut Option<F>, screen: i32) -> Result<(), X11Display>
where
	F: 'static + FnOnce() -> Box<dyn EventHandler>,
{
	...
	let depth = glx.depth;
	display.window = display.libx11.create_window(display.root, display.display, visual, depth, conf);

	let (glx_context, glx_window) = glx.create_context(display.display, display.window);
	glx.swap_interval(display.display, glx_window, glx_context, conf.platform.swap_interval.unwrap_or(1));
	...
	let clipboard = Box::new(clipboard::X11Clipboard::new(display.libx11.clone(), display.display, display.window));
...
unsafe fn egl_main_loop<F>(mut display: X11Display, conf: &crate::conf::Conf, f: &mut Option<F>) -> Result<(), X11Display>
where
	F: 'static + FnOnce() -> Box<dyn EventHandler>,
{
	...
	display.window = display.libx11.create_window(display.root, display.display, std::ptr::null_mut(), 0, conf);

	let (context, config, egl_display) = egl::create_egl_context(&mut egl_lib, display.display as *mut _, conf.platform.framebuffer_alpha, conf.sample_count).unwrap();

	let egl_surface = (egl_lib.eglCreateWindowSurface.unwrap())(egl_display, config, display.window, std::ptr::null_mut());
	...
	crate::native::gl::load_gl_funcs(|proc| {
		...
		egl_lib.eglGetProcAddress.expect("non-null function pointer")(name.as_ptr() as _)
	});
	...
	F: 'static + FnOnce() -> Box<dyn EventHandler>,
{
	unsafe {
		...
		let x11_root = (*(*(x11_display as _XPrivDisplay)).screens.offset(x11_screen as isize)).root;
