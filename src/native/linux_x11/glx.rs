...
pub type PFNGLXCREATENEWCONTEXTPROC = Option<unsafe extern "C" fn(_: *mut Display, _: GLXFBConfig, _: libc::c_int, _: GLXContext, _: libc::c_int) -> GLXContext>;
...
impl LibGlx {
	...
	pub unsafe fn get_procaddr(&self, procname: &str) -> Option<unsafe extern "C" fn()> {
		if self.glxGetProcAddress.is_some() {
			...
			let name = std::ffi::CString::new(procname).unwrap();
			self.glxGetProcAddressARB.expect("non-null function pointer")(name.as_ptr() as _)
		} else {
...
impl Glx {
	pub unsafe fn init(libx11: &mut LibX11, display: *mut Display, screen: i32, conf: &crate::conf::Conf) -> Option<Glx> {
		...
		if (libgl.glxQueryExtension.unwrap())(display, &mut errorbase, &mut eventbase) == 0 {
			...
			crate::error!("GLX: GLX extension not found");
			return None;
		}
		...
		if (libgl.glxQueryVersion.unwrap())(display, &mut glx_major, &mut glx_minor) == 0 {
			#[cfg(feature = "log-impl")]
			crate::error!("GLX: Failed to query GLX version");
			return None;
		}
		if glx_major == 1 && glx_minor < 3 {
			#[cfg(feature = "log-impl")]
			crate::error!("GLX: GLX version 1.3 is required");
			return None;
		}
		...
		assert!(!fbconfig.is_null(), "GLX: Failed to find a suitable GLXFBConfig");

		let result = libgl.glxGetVisualFromFBConfig.unwrap()(display, fbconfig);
		assert!(!result.is_null(), "GLX: Failed to retrieve Visual for GLXFBConfig");
		...
		if extensions_string.contains("GLX_EXT_swap_control") {
			extensions.glxSwapIntervalExt = std::mem::transmute_copy(&libgl.get_procaddr("glXSwapIntervalEXT"));
		}
		if extensions_string.contains("GLX_MESA_swap_control") {
			extensions.glxSwapIntervalMesa = std::mem::transmute_copy(&libgl.get_procaddr("glXSwapIntervalMESA"));
		}

		if extensions_string.contains("GLX_ARB_create_context") && extensions_string.contains("GLX_ARB_create_context_profile") {
			extensions.glxCreateContextAttribsARB = std::mem::transmute_copy(&libgl.get_procaddr("glXCreateContextAttribsARB"))
		};
	...
	pub unsafe fn create_context(&mut self, display: *mut Display, window: Window) -> (GLXContext, GLXWindow) {
		...
		let glx_ctx = self.extensions.glxCreateContextAttribsARB.unwrap()(display, self.fbconfig, std::ptr::null_mut(), true as _, attribs.as_ptr());
		assert!(!glx_ctx.is_null(), "GLX: failed to create GL context");
		// _sapp_x11_release_error_handler(libx11);

		let glx_window = self.libgl.glxCreateWindow.unwrap()(display, self.fbconfig, window, std::ptr::null());
		assert!(glx_window != 0, "GLX: failed to create window");

		(glx_ctx, glx_window)
	}

	pub unsafe fn destroy_context(&mut self, display: *mut Display, window: GLXWindow, ctx: GLXContext) {
		if window != 0 {
	...
	pub unsafe fn make_current(&mut self, display: *mut Display, window: GLXWindow, ctx: GLXContext) {
	...
	pub unsafe fn swap_interval(&mut self, display: *mut Display, window: GLXWindow, ctx: GLXContext, interval: i32) {
		self.libgl.glxMakeCurrent.unwrap()(display, window, ctx);
...
unsafe fn choose_fbconfig(libgl: &mut LibGlx, libx11: &mut super::LibX11, display: *mut Display, screen: i32, multisample: bool, desired_sample_count: i32) -> GLXFBConfig {
	...
	if !vendor.is_null() && libc::strcmp(vendor, b"Chromium\x00" as *const u8 as *const libc::c_char) == 0 as libc::c_int {
	...
	desired.samples = if desired_sample_count > 1 { desired_sample_count } else { 0 };
	closest = gl_choose_fbconfig(&mut desired, usable_configs.as_mut_ptr(), usable_count as libc::c_uint);
...
pub unsafe extern "C" fn gl_choose_fbconfig(desired: *const GLFBConfig, alternatives: *const GLFBConfig, count: libc::c_uint) -> *const GLFBConfig {
	let mut missing;
