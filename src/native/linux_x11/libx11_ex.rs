...
impl LibX11 {
	pub unsafe fn update_system_dpi(&mut self, display: *mut Display) -> f32 {
		...
		if !rms.is_null() {
			...
			if !db.is_null() {
				let mut value = XrmValue { size: 0, addr: 0 as *mut libc::c_char };
				let mut type_ = std::ptr::null_mut();
				if (self.XrmGetResource)(db, b"Xft.dpi\x00".as_ptr() as _, b"Xft.Dpi\x00".as_ptr() as _, &mut type_, &mut value) != 0 {
	...
	pub unsafe fn query_window_size(&mut self, display: *mut Display, window: Window) -> (i32, i32) {
	...
	pub unsafe fn create_window(&mut self, root: Window, display: *mut Display, visual: *mut Visual, depth: libc::c_int, conf: &crate::conf::Conf) -> Window {
